...
try:
    ...
    from session_manager.forge.forge_client import ForgeClient
...
class WebUIServer:
    ...
    def __init__(self, port: int = 8844, session_manager_url: str = "http://localhost:8845"):
        ...
        self.forge_client = ForgeClient(forge_url="http://127.0.0.1:8080", timeout=300)
    ...
    def _execute_forge_processing(self, session_id: str):
        ...
        try:
            # Load the actual trigger JSON file generated by ForgeTriggerGenerator
            trigger_file = Path(f"trigger_{session_id}.json")
            if not trigger_file.exists():
                logger.error(f"Trigger file not found: {trigger_file}")
                return {
                    'status': 'error',
                    'session_id': session_id,
                    'error': f'Trigger file not found: {trigger_file}',
                    'message': 'Could not find generated trigger JSON file'
                }
            
            # Load the complete trigger JSON
            with open(trigger_file, 'r') as f:
                trigger_json = json.load(f)
            
            logger.info(f"Loaded trigger JSON from {trigger_file}")
            
            # Send to real Forge service
            forge_response = self.forge_client.process_session(trigger_json)
